...
- **Follow-up (same day):** all UI renders 50% larger — `CanvasScaler.referenceResolution` changed 1920x1080 → 1280x720 in `Phase5cUIBuilder` (readability request; the builder re-applies it on every rebuild, so don't "fix" it back). Station panels now auto-close when the player walks away: `Scripts/UI/StationProximityCloser.cs` (single-slot tracker on HUDCanvas, close distance = station `InteractRange` + 2u, tunable const); all 5 station UIs (`MerchantUI`/`StorageUI`/`TrainingUI`/`PortalUI`/`CraftingUI`) Track on Open and Untrack on Close — Merchant/Storage/Portal `Open()` signatures gained a station parameter. Equipment/Inventory are player UI, not station UI — they don't auto-close.
