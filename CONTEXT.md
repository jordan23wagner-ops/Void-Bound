...
## Player Death + Gravestone Log (2026-07-04, between Phases 7 and 8 — CODE-REVIEWED, runtime test PENDING)
- **NOTE:** written while the Unity MCP bridge was disconnected from the Claude Code session, so this was thorough code review only — **not yet play-mode verified**. Re-test on death: correct items drop, gravestone spawns/persists, respawn at Homestead, recovery, die-again clears, pool heals, buff text placement.
