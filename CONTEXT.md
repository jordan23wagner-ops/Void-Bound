...
- **Loadout kits + one-click swap** (`DevPlaySetup`, editor-only): Melee (sword+shield), Ranged (bow, no shield), Mage (staff, no shield). Armor is the **shared Iron set for now** (weapon defines the class) — distinct class armor models (leather/robes) are a deferred follow-up. Swap via **VoidBound → Dev → Equip Melee/Ranged/Mage Kit** (also sets the auto-equip default; checkmark shows the current kit). Auto-equip-on-play now equips the selected kit. `EquipKit` clears equipped+backpack then equips by asset path.
